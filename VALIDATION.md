...
- 56 automated tests pass. New cases prove that a higher-level distraction cannot displace the reviewed MK11 core, the opening order is correct, gear is never reused, missing slots are not padded, the Soaked plan is gated to manual Tower bosses, noisy `MKI11` and `MKIT` labels resolve correctly, and older recognized gear is remapped to current strategy profiles.
