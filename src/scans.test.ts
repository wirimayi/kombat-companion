...
import {acceptScans,catalogId,scoringCollection,validateScans} from './scans';
...
describe('automatic screenshot collection',()=>{
 ...
 it('maps older recognized scans to strategy profiles by exact card name',()=>{const old={...scan("Varmint's Lucky Hat",null,'gear'),available:true};expect(catalogId(old)).toBe('varmints-lucky-hat');expect(scoringCollection([old]).gear.some(g=>g.id==='varmints-lucky-hat')).toBe(true);});
 it('uses the vetted plan without filling missing slots with arbitrary gear',()=>{const drafts=[...CHARACTERS.slice(0,9).map(c=>scan(c.name,c.id)),...GEARS.slice(0,16).map(g=>scan(g.name,g.id,'gear'))];const cards=acceptScans([],drafts).cards;const result=recommend(scoringCollection(cards),{mode:'tower',tower:TOWERS[0],difficulty:'normal',realmMode:'quick',playStyle:'manual',useKameo:true}).recommendations[0];expect(result.planName).toBe('MK11 pressure and rescue');expect(result.team).toHaveLength(3);const ids=result.team.flatMap(m=>m.gear.map(g=>g.id));expect(ids).toHaveLength(5);expect(new Set(ids).size).toBe(ids.length);expect(result.warnings.join(' ')).toContain('unavailable');});
