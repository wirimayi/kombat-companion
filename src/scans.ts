...
export function acceptScans(previous:ScanCard[], drafts:OCRDraft[]) {
  ...
  for(const d of drafts){
    ...
    cards.set(key,{kind:d.kind,name:d.name,id:d.id,level:d.level??old?.level??null,fusion:d.fusion??old?.fusion??null,ascension:d.ascension??old?.ascension??null,available:old?.available??d.available??true});
  }
...
export function scoringCollection(scans:ScanCard[],saved:Collection=emptyCollection()):Collection {
  ...
  for(const s of scans){
    if(!s.id || !(s.kind==='gear'?GEARS:CHARACTERS).some(c=>c.id===s.id)) continue;
    if(s.kind==='gear'){
      if(s.fusion!==null)result.gear=result.gear.map(c=>c.id===s.id?{...c,fusion:s.fusion!}:c);
      if(!result.gear.some(c=>c.id===s.id))result.gear.push({id:s.id,fusion:s.fusion??0});
    }else if(s.level!==null){
      if(s.kind==='character')result.characters=result.characters.map(c=>c.id===s.id?{...c,available:s.available,level:s.level!,fusion:s.fusion??c.fusion,ascension:s.ascension??c.ascension}:c);
      if(s.kind==='kameo')result.kameos=result.kameos.map(c=>c.id===s.id?{...c,available:s.available,level:s.level!,fusion:s.fusion??c.fusion}:c);
      if(s.kind==='character'&&!result.characters.some(c=>c.id===s.id))result.characters.push({id:s.id,level:s.level,fusion:s.fusion??(s.level>50?10:0),ascension:s.ascension??0,available:s.available});
      if(s.kind==='kameo'&&!result.kameos.some(c=>c.id===s.id))result.kameos.push({id:s.id,level:s.level,fusion:s.fusion??0,available:s.available});
    }
  }
  return result;
}
