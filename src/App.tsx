...
export default function App() {
  ...
  useEffect(() => {
    let live=true;
    loadCollection().then(c => {if(live && c){setCollection(c);if(c.characters.length)setPage('builder');}}).catch(e => live && setError(`Your collection could not be loaded: ${asError(e)}. Import a backup to recover it.`)).finally(() => live && setReady(true));
...
function CollectionPage({collection,saving,persist,notify,fail,download,exitDemo}:{collection:Collection;saving:boolean;persist:(c:Collection)=>Promise<boolean>;notify:(s:string)=>void;fail:(s:string)=>void;download:()=>void;exitDemo:()=>void}) {
  ...
  const screenshots=async(files:FileList|null)=>{
    ...
    for(const file of Array.from(files).slice(0,60)){
      try {
        ...
        const result=await recognizeScreenshot(file,kind==='kameo'?'kameo':'auto',(p,s)=>{setProgress(p);setStatus(`${file.name} · ${s}`);});
        setDrafts(prev=>[...prev,...result.drafts.map(d=>({...newDraft(d.kind),...d,key:crypto.randomUUID()})).filter(d=>!prev.some(p=>p.kind===d.kind&&p.name===d.name&&p.level===d.level&&p.fusion===d.fusion))]);count++;
  ...
  const saveDraft=async(d:Draft)=>{
    ...
    if(d.kind==='character'){
      ...
      const problems=c.id.startsWith('unverified: ')?[]:validateCharacter(c);if(problems.length){fail(problems.join(' '));return;}
  ...
  return <section className="collection-page"><div className="eyebrow">THE CARDS IN YOUR CORNER</div><div className="collection-heading"><div><h1>My collection.</h1><p className="lead">Upload screenshots. Check the results. You’re ready.</p></div><div className="button-row"><button className="icon-button" title="Export collection backup" aria-label="Export collection backup" onClick={download}><ArrowDownToLine size={19}/></button><button className="icon-button" title="Import collection backup" aria-label="Import collection backup" onClick={()=>backupRef.current?.click()}><ArrowUpFromLine size={19}/></button></div></div>
    ...
    <section className="upload-panel"><div className="upload-icon"><Upload size={23}/></div><div><h3>Start with your screenshots.</h3><p>{kind==='kameo'?'Upload your Kameo screenshots.':'Upload character and gear screenshots together. We’ll identify the card type automatically.'}</p><small>PNG, JPG or WebP · Up to 60 at a time · Processed on your device</small></div><button className="primary" disabled={busy} onClick={()=>fileRef.current?.click()}>{busy?<LoaderCircle size={16} className="spin"/>:<Plus size={17}/>} Upload screenshots</button><input type="file" ref={fileRef} multiple accept="image/png,image/jpeg,image/webp" hidden onChange={e=>{void screenshots(e.target.files);e.target.value='';}}/></section>
    {busy&&<div className="ocr-progress" role="status"><div><span>{status}</span><strong>{Math.round(progress*100)}%</strong></div><progress max="1" value={progress}/><p>Reading each card separately. Grid fusion and ascension need confirmation against the thumbnail. Check ownership and availability before saving.</p></div>}
