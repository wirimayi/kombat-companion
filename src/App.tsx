...
const defaults:Settings={mode:'tower',tower:TOWERS[0],difficulty:'normal',realmMode:'quick',playStyle:'manual',useKameo:true};
...
export default function App(){
 ...
 return <div className="simple-shell"><header><strong>Kombat Companion</strong><span>Private · Free</span></header><main className="simple">
  ...
  <div className="fields">{settings.mode==='tower'&&<label>Tower<select value={settings.tower} onChange={e=>setSettings({...settings,tower:e.target.value})}>{TOWERS.map(t=><option key={t}>{t}</option>)}</select></label>}
  ...
  {results&&<section className="results" aria-live="polite"><div className="result-title"><h2>3. Your suggested team</h2>{results.recommendations.length>1&&<button onClick={()=>setOption((option+1)%results.recommendations.length)}>Another team →</button>}</div>
  {result?<><div className="team">{result.team.map((member,i)=><article key={member.character.id}><small>{i===0?'START WITH':member.role}</small><h3>{fighter(member.character.id)}</h3><ul>{member.gear.map(g=><li key={g.id}><Shield size={14}/>{equipment(g.id)}</li>)}</ul>{!member.gear.length&&<p>Upload more gear screenshots to fill this loadout.</p>}</article>)}</div>{result.kameo&&<p className="kameo"><strong>Kameo:</strong> {fighter(result.kameo.id)}</p>}
  <p className="hint">Suggested from supported cards. Unread fusion uses conservative estimates; it isn’t treated as a confirmed upgrade.</p>
